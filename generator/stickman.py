...
class stick_man_generator(tf.keras.utils.Sequence):
    def __init__(
        ...
        self.key_points = key_points_generator(
            batch_size=batch_size, set_of_data=set_of_data
        )
    ...
    def data_generation(self, index):
        ...
        stickman_colors = np.random.choice(
            len(self.possible_colors), len(KP), replace=True
        )
        self.background.stickman_colors = stickman_colors
        images = self.background.__getitem__(index=index)
        KP_x = np.copy(KP[:, ::2])
        KP_y = np.copy(KP[:, 1::2])
        min_KP_x = np.expand_dims(np.min(KP_x, axis=1), axis=-1)
        min_KP_y = np.expand_dims(np.min(KP_y, axis=1), axis=-1)
        max_KP_x = np.expand_dims(np.max(KP_x, axis=1), axis=-1)
        ...
        for b in range(self.batch_size):
            images[b, ...] = draw_a_stickman(
                ...
                X=KP_x[b, :],
                Y=KP_y[b, :],
                color=self.possible_colors[stickman_colors[b]],
...
def run_visualisation():
    ...
    for i in range(test.__len__()):
        print(
            "\rvisualisation : batch (2x224x224x3) %i/%i         "
            % (i + 1, test.__len__()),
            end="",
        )
        tmp = time.time()
        ...
        for b in range(len(imgs)):
            ...
            if k == 27:  # Esc key to stop
                ...
                print(
                    "\rvisualisation : batch (2x224x224x3) %i/%i (paused)"
                    % (i + 1, test.__len__()),
                    end="",
                )
                k = cv2.waitKey(33)
    ...
    for i in range(test.__len__()):
        print(
            "\rvisualisation : batch (128x1920x1080x3) %i/%i" % (i + 1, test.__len__()),
            end="",
        )
        tmp = time.time()
        imgs = test.__getitem__(i)[0]
